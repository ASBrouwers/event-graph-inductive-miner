...
sampleFileName = "BPIC17_Sample_320cases"
numberOfCases = 320
...
def CreateBPI17(inputpath, path_to_neo4j_import_directory, fileName, sample):
    ...
    if (sample == True): 
        sIds = csvLog['case'].unique().tolist()
        # print(len(sIds)) # Total number of cases: 31509

        percentage = 2.5 # Sample of n%
        k = len(sIds) * percentage // 100
        k = round(k)

        random.seed(1)
        sampleIds = random.sample(sIds, numberOfCases) # Sample of n cases
        # sampleIds = random.sample(sIds, k)
        
        with open(inputpath+sampleFileName+'.txt', 'w') as f:
            for id in sampleIds:
                f.write("%s\n" % id)
    ...
    logSamples.to_csv(path_to_neo4j_import_directory+fileName, index=True, index_label="idx")


if(sample):
    # fileName = 'BPIC17sample.csv' 
    fileName = sampleFileName+'.csv'
